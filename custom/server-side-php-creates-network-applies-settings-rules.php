...
$res = $conn->query("SELECT * FROM `z5Z7XTD_users` AS u INNER JOIN `z5Z7XTD_bp_xprofile_data` AS d ON u.id = d.user_id where d.field_id = 13 AND u.flag_NetworkCreated = 0");
while ($row = $res->fetch_assoc()) {
    echo " user = " . $row['user_email'] . "\n";
    echo " PIN = " . $row['value'] . "\n";
    echo " NetworkCreated = " . $row['flag_NetworkCreated'] . "\n";
    echo " uid = " . $row['user_id'] . "\n";
    echo " lastUpdated = " . $row['last_updated'] . "\n";
    if ($row['flag_NetworkCreated'] == 0) {
        ...
        $conn->query($wb);
        $wb = "UPDATE `z5Z7XTD_bp_xprofile_data` SET `cron_timecompare` = \"$lastUpdated\" WHERE `field_id` = 13 AND `user_id` = \"$uid\"";
        $conn->query($wb);
    }
}
CloseCon($conn);
