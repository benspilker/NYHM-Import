...
Write-Host " "
Write-Host " "
Invoke-WebRequest -Uri "https://www.dropbox.com/s/e1pwshque56l7xv/nowyouhearme.zip?dl=1" -OutFile "$ENV:UserProfile\AppData\Local\Temp\nowyouhearme.zip"
...
xcopy /y /E /c "$ENV:UserProfile\AppData\Local\Temp\nyhm-update" "C:\Program Files (x86)\" >$null 2>&1
Remove-Item -path "$ENV:UserProfile\AppData\Local\Temp\nyhm-update" -recurse
